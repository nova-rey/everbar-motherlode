...
def _profile(records: list[dict[str, Any]]) -> dict[str, Any]:
    lengths=[len(ids) for row in records for ids in row["bar_ids"]]; output={"schema":"dreamstream-everbar.bar-profile/v1","scope":"EV1_PREVIEW_ONLY","token_length_distribution":_percentiles(lengths),"candidate_caps":{}}
    chosen=None
    for cap in CAPS:
...
def build(*, motherlode_root: Path, output_root: Path, everbar_checkout: Path, motherlode_sha: str) -> dict[str, Any]:
    pdmx_dbs=_completed_pdmx_dbs(motherlode_root); pop_db=motherlode_root/"state"/"motherlode.sqlite"
    all_rows=[entry for path in pdmx_dbs for entry in _rows(path,"pdmx")] + _rows(pop_db,"pop909")
    ...
    name=f"ev1-preview-clean-{len(selected)//1000}k-v1"; final=output_root/name; stage=output_root/("."+name+".building")
    ...
    for number,entry in enumerate(selected,1):
        token=_tokenize(entry,everbar_checkout); token["source_family_id"]=entry["source_family_id"]; token["split"]=entry["split"]; tokens.append(token)
        if number % 250 == 0: _write(stage/"progress.json",{"state":"TOKENIZING","completed":number,"total":len(selected)})
    profile=_profile(tokens); cap=int(profile["selected_cap"]); _write(stage/"profiles"/"brick4.json",profile)
